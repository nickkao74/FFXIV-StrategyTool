...
| `07-UWU-P1對照.md` | UWU P1 迦樓羅:逐字稿 / wiki / BossMod 三方對照與衝突判定 |
| `08-UWU-P2對照.md` | UWU P2 伊弗利特:尺寸校對(修正幅度最大的一次),以及 BossMod 反向佐證的三個結論 |
...
- [x] 流程套用到 UWU P1(產出校對版 `uwu-lab`,見 `07`)
- [x] 流程套用到 UWU P2(見 `08`;產生器同時移植為 Python)
- [ ] 檢視 `uwu-lab`,決定是否取代正式版的 `uwu`
- [ ] UWU P2 以後(參考資料已備齊,圖解尚未撰寫)

## 有攻略逐字稿時的優先序

朱雀那次只有插件單一來源;UWU 開始有第一手的攻略逐字稿與 wiki,
因此**加上一條凌駕於可信度分級之上的規則**:

| 序 | 來源 | 角色 |
|:--:|---|---|
| 1 | 攻略逐字稿(附實戰截圖) | 機制描述與打法的最終依據 |
| 2 | 遊戲 wiki | 補完數值與細則 |
| 3 | BossMod | **只用來調整幾何尺寸** |

BossMod 的 A 級只代表「這是插件實際執行的數值」,不代表「這是機制的真相」——
插件作者一樣會建模錯誤,而且他們會用 `// TODO: verify` 誠實標出來(UWU 的寒風之歌就是)。
**幾何尺寸信 BossMod,機制語意信逐字稿。**
